...
contract ReservationMarketplace is Ownable, IERC721Receiver, ReentrancyGuard {
	...
	event NFTSold(uint indexed tokenId, address indexed seller, address indexed buyer, uint price, uint fee);
	...
	function listNFT(uint _tokenId, uint _price) public payable nonReentrant {
		require(nftContract.ownerOf(_tokenId) == msg.sender, "Only token owner can list");
		require(listings[_tokenId].seller == address(0), "Token already listed");
		require(_price > 0, "Price must be greater than zero");
		require(nftContract.isApprovedForAll(msg.sender, address(this)) == true, "Marketplace not approved to transfer token");
		uint tokenDateMaxToSell = nftContract.getDateMaxToTrade(_tokenId);
		require(tokenDateMaxToSell < block.timestamp, "Token can't be traded anymore");
	 ...
	 */
	function getAllListingsBySeller(address _seller) external view returns (uint[] memory) {
	 ...
	 * @param _seller The seller address
	 */
	function getAllActiveListingsBySeller(address _seller) external view returns (uint[] memory) {
		uint[] memory result = new uint[](sellerListings[_seller].length());
		uint count = 0;
		for (uint i = 0; i < sellerListings[_seller].length(); i++) {
	 ...
	 */
	function getValueWithFeeApplied(address seller, uint price) public view returns (uint) {
